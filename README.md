...
- **Auditoría de rediseño para escritorio (pendiente, prioridad alta)**: el diseño de `css/`/`js/views/` está hecho *mobile-first* (bottom nav, tarjetas apiladas, tipografía y espaciados pensados para pantalla de móvil en vertical). En una ventana de escritorio Windows (ratón + teclado, ventana ancha y redimensionable, sin gestos táctiles) la experiencia actual previsiblemente se ve desaprovechada: contenido angosto y centrado, navegación inferior poco natural con ratón, sin aprovechamiento del ancho horizontal ni de patrones de escritorio (barra lateral, tablas densas, atajos de teclado, tamaños de clic más precisos). Falta hacer una auditoría completa de UI/UX específica para este entorno (breakpoints de escritorio, layout alternativo ≥1024px, revisión de `bottom-nav`/`more-sheet` para ratón) antes de considerar la app "verdaderamente lista" para Microsoft Store, más allá de que técnicamente funcione.
